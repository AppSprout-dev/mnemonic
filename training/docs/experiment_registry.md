...
- **Result:** Best eval loss 1.2002 (PPL 3.3), improvement of -0.483 from init (1.6830). Below MI300X init (1.2030). **However, generation quality is poor.** Evaluation on 13/25 gold probes: 2/13 valid JSON (15%), 0 SC. The base model without spokes (EXP-29) achieves 24/25 valid JSON zero-shot. Spokes made generation worse.
- **Failure mode:** Training optimized teacher-forced next-token prediction — the model learned to predict the next token given ground-truth context (eval loss 1.20). But autoregressive generation compounds spoke perturbations: each spoke-modified hidden state feeds into the next token prediction, which feeds back through the spokes, creating a degradation cascade. The model produces repetition loops and malformed output despite low eval loss.
- **Root cause analysis:** NF4 quantization noise in the base model's hidden states is amplified by the spoke → generate → spoke feedback loop. Gates at 0.12-0.88 inject substantial spoke signal into the residual stream. With bf16 base weights (MI300X), the hidden states are clean enough for this to work. With NF4, the dequantization noise compounds through the autoregressive loop.
- **Verdict:** REFUTED. Spoke training on NF4-quantized Gemma E2B produces models that minimize eval loss but cannot generate coherent structured output. The teacher-forced loss metric is misleading — it does not predict generation quality for spoke adapters on quantized models. **GBNF grammar enforcement on the base model (no spokes) is the production path for Gemma E2B on consumer hardware.** NF4 spoke training may work with much lower gate values (< 0.05) to reduce the perturbation magnitude, but this is speculative.
- **Implications:**
  1. NF4 + spokes is not viable for generation tasks at current gate scales. Confirmed: NF4 gradient noise creates an eval loss floor (~1.20) that does not translate to usable output.
  2. Gemma E2B + faithful prompt + GBNF grammar (no spokes, no training) is the production deployment path.
  3. Spoke training for Gemma requires full bf16 (MI300X or equivalent). The EXP-20 result (0.6082 eval loss on MI300X, 7/7 stress test) remains the gold standard.
  4. For local deployment on the RX 7800 XT: use the Qwen 3.5 2B spokes (EXP-18, 100% SC, 95 tok/s) or Gemma E2B base + GBNF (no spokes).
