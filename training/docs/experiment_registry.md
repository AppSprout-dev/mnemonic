...
- **Verdict:** REFUTED. The null hypothesis is confirmed: spoke training on NF4-quantized Gemma E2B degrades output quality compared to the base model + faithful prompt alone. The spokes inject enough perturbation to disrupt the base model's JSON generation ability while not learning enough schema structure to compensate. **Eval loss is not a reliable proxy for generation quality with NF4 spoke adapters.** Production path: Gemma E2B + faithful prompt + GBNF grammar (no spokes). Spoke training requires full bf16 (MI300X).
- **Key learning:** NF4 quantization noise compounds through the autoregressive generation loop when amplified by spoke perturbations (gates at 0.12-0.88). The spokes learned teacher-forced next-token prediction but the dequantization noise creates a mismatch between training (clean teacher-forced context) and inference (spoke-perturbed context feeding back into the next step). Lower gate values or bf16 base weights would reduce this effect.
