...
def build_production_prompt(
    ...
    Uses the "faithful" prompt format (EXP-29): rules-first design that leads
    with FAITHFULNESS, PRESERVATION, MINIMALITY before the schema. This format
    produced 100% EPR, 100% NP on Gemma 4 E2B zero-shot.

    The concept_vocabulary parameter is accepted for API compatibility but
    no longer injected into the prompt (EXP-29 showed it hurt faithfulness).
    """
    parts = []

    if source == "ingest":
        parts.append(
            "TASK: Catalog this source code file into structured JSON.\n\n"
            "RULES:\n"
            "- FAITHFULNESS: Every fact in your output must come from the file content. "
            "Do not infer, speculate, or add context from your training data.\n"
            "- PRESERVATION: Copy all function names, type names, file paths, import paths, "
            "and version strings VERBATIM from the file.\n"
            ...
            "- structured_concepts: {topics: [{label, path}], entities: [{name, type, context}], "
