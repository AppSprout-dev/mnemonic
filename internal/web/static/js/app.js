    ...
    function handleHash() {
        ...
        if (hash.startsWith('forum-category/')) {
            ...
            if (catId) loadForumCategory(catId, catId);
            return;
        }
    ...
    async function loadThread(episodeId) {
        ...
        try {
            ...
            if (memories.length > 0) {
                ...
                try {
                    var assocResp = await forumFetch('/api/v1/associations?memory_ids=' + memIds);
    ...
    function renderTimelineCard(item, idx) {
        var kind = item._kind;
    ...
    function expandTimelineCard(el) {
        ...
        ws.onmessage = function(event) {
            try {
                ...
                if (state.currentView === 'explore') {
                    ...
                    if (!state.exploreLoaded[tab]) loadExploreTab(tab);
                    if (!state.forumLoaded) loadForumIndex();
                }
                // Track forum notifications
                if (type === 'forum_post_created') onForumPostWebSocket(payload);
                ...
                if (type === 'forum_post_created' && state.currentView === 'thread' && state.currentForumThread === payload.thread_id) {
                    appendForumPostToThread(payload);
                }
    ...
    function linScale(val, dMin, dMax, rMin, rMax) {
    ...
    function memoryType(m) {
        var raw = (m.type || m.memory_type || 'general').toLowerCase();
        return _sourceTypeMap[raw] || raw;
    }
    function memoryTypeAbbr(type) { return type === 'insight' ? 'IN' : type === 'decision' ? 'DE' : type === 'learning' ? 'LE' : type === 'error' ? 'ER' : 'GN'; }
    function memoryTypeIcon(type) { return type === 'insight' ? 'icon-in' : type === 'decision' ? 'icon-de' : type === 'learning' ? 'icon-le' : type === 'error' ? 'icon-er' : 'icon-ep'; }
    function safeSalience(s) { return Math.min(100, Math.round((s || 0) * 100)); }
    ...
    function addLivePost(wsType, payload, timestamp) {
    ...
    async function loadMemorySection(sectionId, sectionName) {
        state.currentView = 'thread';
        ...
        try {
            if (sectionId === 'episodes') {
                ...
                for (var i = 0; i < eps.length; i++) {
                    ...
                    html += '<li class="row ' + bgClass + '" onclick="loadThread(\'' + ep.id + '\')" style="cursor:pointer">';
                    html += '<dl class="row-item"><dt>';
                ...
                for (var i = 0; i < mems.length; i++) {
                    ...
                    if (m.episode_id) html += '<br><a onclick="event.stopPropagation(); loadThread(\'' + m.episode_id + '\')" style="color:var(--accent-cyan);cursor:pointer">View episode thread →</a>';
                    html += '</div>';
                    html += '</div></dt>';
    ...
    async function loadForumGroup(type) {
        ...
        var bc = document.getElementById('breadcrumbs');
        if (bc) bc.innerHTML = '<a href="#" onclick="switchView(\'explore\')">mnemonic</a><span class="sep"> › </span><a href="#" onclick="switchView(\'explore\')">Forum</a><span class="sep"> › </span>' + escapeHtml(groupName);
        var compose = document.getElementById('threadCompose');
        if (compose) compose.style.display = 'none';
    ...
    async function loadForumCategory(categoryId, categoryName) {
        ...
        var bc = document.getElementById('breadcrumbs');
        if (bc) bc.innerHTML = '<a href="#" onclick="switchView(\'explore\')">mnemonic</a><span class="sep"> › </span><a href="#" onclick="switchView(\'explore\')">Forum</a><span class="sep"> › </span>' + escapeHtml(categoryName);
    ...
    async function loadForumThread(threadId) {
        ...
        var bc = document.getElementById('breadcrumbs');
        if (bc) bc.innerHTML = '<a href="#" onclick="switchView(\'explore\')">mnemonic</a><span class="sep"> › </span><a href="#" onclick="switchView(\'explore\')">Forum</a><span class="sep"> › </span>Thread';
    ...
    function renderForumPost(post, index) {
    ...
    function appendForumPostToThread(payload) {
        var container = document.getElementById('threadContent');
        if (!container) return;
    ...
    function quotePostById(postElementId) {
    ...
    function insertTagInReply(agentKey) {
        var textarea = document.getElementById('threadReplyContent');
        if (!textarea) return;
        var tag = '@' + agentKey + ' ';
    ...
    function insertMention(textareaId, agentKey) {
    ...
    function ensureMentionAutocomplete(textareaId) {
    ...
    function onForumPostWebSocket(payload) {
    ...
    async function internalizePost(postId, btn) {
        btn.disabled = true;
