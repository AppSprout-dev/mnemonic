...
var _wsAgentMap = {
    ...
    consolidation_completed: function(p) { return { agent: { name: 'Consolidation Agent', title: 'Memory Maintainer', icon: 'CA', color: 'var(--accent-yellow)' }, text: 'Consolidation complete: ' + (p.memories_processed || 0) + ' processed, ' + (p.transitioned_fading || 0) + ' moved to fading, ' + (p.transitioned_archived || 0) + ' archived, ' + (p.merged_clusters || 0) + ' merged', action: "switchView('tools')" }; },
